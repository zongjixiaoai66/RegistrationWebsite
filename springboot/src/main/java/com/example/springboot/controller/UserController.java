...
//        }
    ...
    public Result findPageByStudent(@RequestParam Integer pageNum,
                           @RequestParam Integer pageSize,
        ...
        if("ROLE_TEACHER".equals(byId.getRole())){
    ...
    public Result members() {
        ...
        int q1 = arriveschoolService.list().size(); // 已报到人数
        QueryWrapper<User> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("role","ROLE_USER");
        int total = userService.list(queryWrapper).size();
        int q2 = total - q1; // 未报到人数
     ...
     * @return
     */
    @GetMapping("/pageByDorm")
    public Result pageByDorm(@RequestParam Integer pageNum,
                           @RequestParam Integer pageSize,
                           @RequestParam Integer userid,
                           @RequestParam(defaultValue = "") String username) {
...
//        List<User> list = reader.readAll(User.class);
    ...
    @AuthAccess
    public Result importTeacher(MultipartFile file) throws Exception {
        InputStream inputStream = file.getInputStream();
        ExcelReader reader = ExcelUtil.getReader(inputStream);
        // 方式1：(推荐) 通过 javabean的方式读取Excel内的对象，但是要求表头必须是英文，跟javabean的属性要对应起来
//        List<User> list = reader.readAll(User.class);
        ...
        for (List<Object> row : list) {
            ...
            user.setAddress(row.get(5).toString());
            user.setAvatarUrl(row.get(6).toString());
            user.setCardnumber(row.get(7).toString());
